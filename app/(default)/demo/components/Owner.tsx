...
export default function Owner() {
  ...
  const handleEmailSubmit = async (email: string) => {
    try {
      const response = await axios.post(
        ...
        {
          to: email,
          subject: "Fact Checker ZKP Mail",
          body: "You are authorised to use Schrodingers Cat",
        },
        {
          headers: {
  ...
  const onFinish = async (values: any) => {
    ...
    // Updated researcher and email handling
    for (let i = 0; i < researcherAccounts.length; i++) {
      const researcher = researcherAccounts[i];
      if (values[researcher["account_name"] + "type"] == undefined) {
        let res = await handleMintResearcher(
          researcher["address"],
          researcher["access_types"]
        );
        tracker.push(res);
      } else {
        let res = await handleMintResearcher(
          researcher["address"],
          values[researcher["account_name"] + "type"]
        );
        tracker.push(res);
      }
      ...
      if (
        showEmailField[researcher["account_name"]] &&
        values[researcher["account_name"]].email
      ) {
        await handleEmailSubmit(values[researcher["account_name"]].email);
      }
    }
  ...
  {researcherAccounts.map((val) => (
    <div style={{ display: "inline-flex", flexDirection: "column", gap: "8px", width: "100%" }}>
      ...
      <Form.Item>
        <Checkbox
          onChange={(e) =>
            setShowEmailField({
              ...showEmailField,
              [val["account_name"]]: e.target.checked,
            })
          }
        >
      ...
      {showEmailField[val["account_name"]] && (
        <Form.Item
          label="Email"
          name={[val["account_name"], "email"]}
